...
const Login = ( {navigation} ) => {
  ...
  const handleLogin = async (values) => {
    ...
    try {
      ...
      const currentUser = userCredential.users;
  
      if (currentUser) {
        const adminEmail = 'admin@example.com'; // Đây là email của tài khoản admin
  
        if (email === adminEmail) {
          navigation.navigate('AdminScreen');
          return; // Chuyển hướng trực tiếp đến trang admin và kết thúc hàm handleLogin
        }
  
        const isAdmin = currentUser.email === adminEmail; // Kiểm tra trực tiếp từ thông tin đăng nhập
