    ...
    const index = selectedSeats.findIndex(
      (selectedSeat) =>
        ...
        (seat.type === 'VIP' &&
          selectedSeats.some(
            (selectedSeat) =>
              selectedSeat.row === seat.row && (selectedSeat.number === seat.number || selectedSeat.number === seat.number + 1)
          ))
    );

    if (index !== -1) {
      setSelectedSeats(selectedSeats.filter((selectedSeat) => selectedSeat !== selectedSeats[index]));
    } else {
