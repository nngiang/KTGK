  ...
  const handleSignup = async (values) => {
    ...
    try {
      if (
        ...
        // Sử dụng Firebase Authentication để tạo tài khoản người dùng với email và password
        ...
        if (currentUser) {
          ...
          // Lưu thông tin fullName vào Firestore hoặc Realtime Database
          await firestore().collection('users').doc(currentUser.uid).set({
            fullName: fullName.trim(),
            // Các thông tin khác có thể được thêm vào đây nếu cần
          });
  
          console.log('Đã đăng ký thành công');
          // Chuyển hướng người dùng đến màn hình đăng nhập sau khi đăng ký thành công
