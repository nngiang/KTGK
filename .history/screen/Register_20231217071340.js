...
const Register = ( {navigation} ) => {
  ...
  const handleSignup = async (values) => {
    ...
    Auth.signUp(fullName, email, password, confirmPassword, navigation) // Truyền navigation vào hàm signUp
      .then(() => {
        navigation.navigate('Login'); // Chuyển hướng về màn hình đăng nhập sau khi đăng ký thành công
      })
      .catch(error => {
        Alert.alert('Email đã có người sử dụng');
      });
  };
