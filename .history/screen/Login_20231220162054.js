    ...
    try {
      ...
      if (currentUser) {
        ...
        if (doc.exists) {
          ...
          if (isAdmin) {
            navigation.navigate('AdminScreen'); // Chuyển hướng đến trang Admin nếu là admin
          } else {
            navigation.navigate('HomeScreen'); // Chuyển hướng đến trang Home nếu không phải admin
          }
        } else {
          navigation.navigate('HomeScreen'); // Chuyển hướng đến trang Home nếu không tìm thấy thông tin người dùng trong collection 'admin'
        }
      }
    } catch (error) {
