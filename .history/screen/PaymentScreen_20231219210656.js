...
const PaymentScreen = ({ route }) => {
  ...
  const generateQRCodeData = () => {
    ...
    const randomNumber = Math.floor(Math.random() * 10) + 1; // Tạo số ngẫu nhiên từ 1 đến 1,000,000
    const qrCodeData = `Booking-${randomNumber}`; // Chuỗi dữ liệu ngẫu nhiên cho mã QR code
    return qrCodeData;
  };
