...
const Login = ( {navigation} ) => {
  ...
  const handleLogin = async (values) => {
    ...
    try {
      ...
      if (currentUser) {
        const adminRef = firestore().collection('users').doc(currentUser.uid).where('isAdmin', '==', true);;
        const doc = await adminRef.get();
  
        if (doc.exists) {
          const userData = doc.data();
          const isAdmin = userData.isAdmin || false; // Thay đổi key tương ứng trong document của users collection
