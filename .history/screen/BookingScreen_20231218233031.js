...
const BookingScreen = () => {
  ...
  const [vipSeats, setVIPSeats] = useState([1, 2, 3, 11, 12, 13]); // Ghế VIP
  const [normalSeats, setNormalSeats] = useState([4, 5, 6, 14, 15, 16]); // Ghế thường
  const [economySeats, setEconomySeats] = useState([7, 8, 9, 10, 17, 18, 19, 20]); // Ghế hạng thấp

  const getSeatLabel = (seatNumber) => {
    const row = Math.ceil(seatNumber / 10); // Số hàng
    const seatIndex = seatNumber % 10 || 10; // Số ghế trong hàng
    const letter = String.fromCharCode(64 + row); // Chữ cái tương ứng với số hàng, 'A' = 65, 'B' = 66...
