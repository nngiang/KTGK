...
const getUserInformation = async (email) => {
  try {
    const usersCollection = firestore().collection('admin'); // Thay 'users' bằng tên collection của bạn
...
// Sử dụng hàm `getUserInformation` trong phần xử lý đăng nhập
  ...
  try {
    ...
    if (currentUser) {
      ...
      if (userInformation && userInformation.isAdmin) {
        ...
        // Ngược lại, chuyển hướng đến trang người dùng thông thường
